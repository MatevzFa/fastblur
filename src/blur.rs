...
pub fn gaussian_blur(data: &mut Vec<f32>, width: usize, height: usize, blur_radius: f32) {
...
fn box_blur_vert(
    ...
    for i in 0..width {
        ...
        let fv: f32 = backbuf[col_start];
        let lv: f32 = backbuf[col_end];

        let mut val_r: f32 = (blur_radius as f32 + 1.0) * f32::from(fv);
        ...
        if blur_radius > height {
            val_r += (blur_radius - height) as f32 * f32::from(lv);
        }

        for _ in 0..min(height, blur_radius + 1) {
            let bb = get_bottom(ri);
            ri += width;
            val_r += f32::from(bb) - f32::from(fv);

            frontbuf[ti] = val_r as f32 * iarr;
            ti += width;
        }
        ...
        if height > blur_radius {
            ...
            for _ in (blur_radius + 1)..(height - blur_radius) {
                ...
                val_r += f32::from(bb1) - f32::from(bb2);
...
fn box_blur_horz(
    ...
    for i in 0..height {
        ...
        let fv: f32 = backbuf[row_start];
        let lv: f32 = backbuf[row_end]; // VERTICAL: $backbuf[ti + $width - 1];
